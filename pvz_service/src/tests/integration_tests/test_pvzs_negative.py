...
async def test_get_pvz_not_found(client):
    ...
    response = await client.get(f"{URL}/{non_existent_id}")
...
async def test_update_pvz_not_found(client):
    ...
    response = await client.patch(f"{URL}/{non_existent_id}", json=update_data)
...
async def test_delete_pvz_not_found(client):
    ...
    response = await client.delete(f"{URL}/{non_existent_id}")
...
async def test_create_pvz_duplicate_code(client, session):
    ...
    response = await client.post(f"{URL}", json=new_pvz_payload.model_dump(mode="json"))
...
async def test_create_pvz_validation_error(client, payload, error_field):
    ...
    response = await client.post(f"{URL}", json=final_payload)
...
async def test_get_employees_pvz_not_found(client):
    ...
    response = await client.get(f"{URL}/{non_existent_id}/employees", params={"user_id": non_existent_id})
