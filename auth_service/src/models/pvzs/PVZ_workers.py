...
class PVZWorkers(Base):
    ...
    pvz_id: Mapped[int] = mapped_column(Integer, ForeignKey("pvz.id"), nullable=False, index=True)
    worker_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    assigned_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    pvz: Mapped["PVZs"] = relationship("PVZ", back_populates="worker_links", lazy="joined")
    worker: Mapped["Users"] = relationship("User", back_populates="pvz_worker_links", lazy="joined")
